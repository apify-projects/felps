...
// export type StepOptionsHandler<Methods = unknown, OtherMethods = unknown, Context extends StepBaseApiContext = any> = (context: RequestContext, api: StepBaseApi<Context> & Without<OtherMethods, Methods>) => Promise<void>

export type GenerateStepGoToMethods<T, Models extends Record<string, unknown> = {}> = {
  [K in Extract<T, string> as `goto${SnakeToPascalCase<K>}`]: (request: RequestSource, references: References<Models>) => void;
};

export type GenerateStepMethods<T, Models extends Record<string, unknown> = {}> = GenerateStepGoToMethods<T, Models>;


// step-base-api.ts ------------------------------------------------------------
export type StepBaseApiMethods<Context extends MakeStepBaseApiContext = any> = StepBaseRootMethods<Context> & HookMethods;

export type StepBaseRootMethods<Context extends MakeStepBaseApiContext = any> = {
  // step
  uid: string,
  name: string,
  id: string
...
export type StepCustomApiExtend<InitialMethods = unknown, Methods = unknown> = (crawlingContext: RequestContext, api: Without<InitialMethods, Methods>) => Methods
...
export type GenerateModelMethods<T extends Record<string, unknown>> = GenerateModelAddMethods<T> & GenerateModelAddPartialMethods<T> & GenerateModelGetReferencePartialMethods<T>;
...
// hooks.ts ------------------------------------------------------------
export type DefaultHookNames = 'STEP_STARTED' | 'STEP_ENDED' | 'STEP_FAILED' | 'STEP_REQUEST_FAILED' | 'ACTOR_STARTED' | 'ACTOR_ENDED' | 'QUEUE_STARTED' | 'QUEUE_ENDED'
